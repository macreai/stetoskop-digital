...
public class MFCC {
    ...
    private static double    sampleRate           = 8000.0;
...
//    }
    ...
    public float[] process(byte[] audioData) {
        ...
        double[] doubleInputBuffer = convertByteArrayToDoubleArray(audioData);
    ...
    private double[] convertByteArrayToDoubleArray(byte[] byteArray) {
        int byteSize = 2; // Assuming 16-bit PCM audio
        int bufferSize = byteArray.length / byteSize;
        double[] doubleArray = new double[bufferSize];

        ByteBuffer byteBuffer = ByteBuffer.wrap(byteArray);
        byteBuffer.order(ByteOrder.LITTLE_ENDIAN); // Assuming little-endian encoding

        for (int i = 0; i < bufferSize; i++) {
            doubleArray[i] = byteBuffer.getShort() / 32768.0; // Normalize to the range [-1, 1]
        }
